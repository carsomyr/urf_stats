...
ActiveRecord::Base.transaction do
  ...
  client.champions.body["data"].each do |_, champion_info|
    champion = Riot::Api::Champion.find_or_initialize_by(entity_id: champion_info["id"].to_i)
  ...
  client.items.body["data"].each do |_, item_info|
    item = Riot::Api::Item.find_or_initialize_by(entity_id: item_info["id"].to_i)
  ...
  client.masteries.body["data"].each do |_, mastery_info|
    mastery = Riot::Api::Mastery.find_or_initialize_by(entity_id: mastery_info["id"].to_i)
  ...
  client.runes.body["data"].each do |_, rune_info|
    rune = Riot::Api::Rune.find_or_initialize_by(entity_id: rune_info["id"].to_i)
